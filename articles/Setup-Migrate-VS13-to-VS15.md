...
```
* Download the file [CNTKCustomMKL-Windows-3.zip](https://msrstagingredirect.azurewebsites.net/en-us/security-risk-detection/download-toolkit/). Unzip it into your CNTK MKL path, creating a numbered sub directory within. For example, if you are on latest master, download and extract its contents to `c:\local\CNTKCustomMKL\3` (the top-level folder inside the ZIP archive is called `3`). For more information, see the [CNTK web site](https://msrstagingredirect.azurewebsites.net/en-us/security-risk-detection/download-toolkit/).
