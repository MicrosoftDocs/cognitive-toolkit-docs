...
## Creating a compiled Protobuf 3.5.2 for CNTK

If you want to build the Protobuf 3.5.2 for CNTK yourself, follow these instructions. The compiled Protobuf version will be located in `c:\local\protobuf-3.5.2-vs17`

* Install CMake from [cmake.org](https://cmake.org/download/).
* Download and unpack the protobuf sources from https://github.com/google/protobuf/archive/v3.5.2.zip.

The following steps assume that the Protobuf sources have been unzipped to the directory `c:\local\src\protobuf-3.5.2`. The instructions below will create the Protobuf libraries and header files in the directory `c:\local\protobuf-3.5.2-vs17`. If this location differs in your case, please adapt the steps below to match your directory structure.
...
buildProtoVS17 C:\local\src\protobuf-3.5.2 C:\local\protobuf-3.5.2-vs17
```
...
```
cd C:\local\src\protobuf-3.5.2\cmake
mkdir build && cd build
```
3. Create a debug build: 
```
mkdir debug && cd debug
cmake -G "NMake Makefiles" -DCMAKE_BUILD_TYPE=Debug -Dprotobuf_BUILD_TESTS=OFF -Dprotobuf_MSVC_STATIC_RUNTIME=OFF -DCMAKE_INSTALL_PREFIX="c:\local\protobuf-3.5.2-vs17" ..\..
nmake && nmake install
cd ..
```
...
cmake -G "NMake Makefiles" -DCMAKE_BUILD_TYPE=Release -Dprotobuf_BUILD_TESTS=OFF -Dprotobuf_MSVC_STATIC_RUNTIME=OFF -DCMAKE_INSTALL_PREFIX="c:\local\protobuf-3.5.2-vs17" ..\..
nmake && nmake install
cd ..
```
