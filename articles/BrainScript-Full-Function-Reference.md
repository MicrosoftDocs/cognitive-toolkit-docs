...
#### Layers
 - [`DenseLayer`](./Layers-Reference#denselayer-linearlayer)` {outDim, bias= true, activation=Identity, init='uniform', initValueScale=1}`  
 - [`ConvolutionalLayer`](./Layers-Reference#convolutionallayer)` {numOutputChannels, filterShape, activation = Identity,`  
                      `init = "uniform", initValueScale = 1,`  
                      `stride = 1, pad = false, lowerPad = 0, upperPad = 0,` 
                      `bias=true}`  
 - [`MaxPoolingLayer`](./Layers-Reference#maxpoolinglayer-averagepoolinglayer)` {filterShape, stride = 1, pad = false, lowerPad = 0, upperPad = 0}`  
 - [`AveragePoolingLayer`](./Layers-Reference#maxpoolinglayer-averagepoolinglayer)` {filterShape, stride = 1, pad = false, lowerPad = 0, upperPad = 0}`  
 - [`EmbeddingLayer`](./Layers-Reference#embeddinglayer)` {outDim, embeddingPath = '', transpose = false}`  
 - [`RecurrentLSTMLayer`](./Layers-Reference#recurrentlstmlayer)` {outputDim, cellShape = None, goBackwards = false, enableSelfStabilization = false}`  
 - [`DelayLayer`](./Layers-Reference#delaylayer)` {T=1, defaultHiddenActivation=0}`  
 - [`Dropout`](./Dropout)  
 - [`BatchNormalizationLayer`](./Layers-Reference#batchnormalizationlayer-layernormalizationlayer-stabilizerlayer)` {spatialRank = 0, initialScale = 1,  
                         normalizationTimeConstant = 0, blendTimeConstant = 0,  
                         epsilon = 0.00001, useCntkEngine = true}`  
 - [`LayerNormalizationLayer`](./Layers-Reference#batchnormalizationlayer-layernormalizationlayer-stabilizerlayer)` {initialScale = 1, initialBias = 0}`  
 - [`StabilizerLayer{}`](./Layers-Reference#batchnormalizationlayer-layernormalizationlayer-stabilizerlayer)  
 - [`FeatureMVNLayer{}`](./Layers-Reference#featuremvnlayer)  

#### Layer building
 - [`Sequential()`](./Sequential)  

#### Activation functions
 - [`Sigmoid`](./Activation-Functions)`(x)`  
 - [`Tanh`](./Activation-Functions)`(x)`  
 - [`ReLU`](./Activation-Functions)`(x)`  
 - [`Softmax`](./Activation-Functions)`(x, axis=0)`  
 - [`LogSoftmax`](./Activation-Functions)`(x)`  
 - [`Hardmax`](./Activation-Functions)`(x)`  

#### Elementwise operations, unary
 - [`Abs`](./Unary-Operations)` (x)`  
 - [`Ceil`](./Unary-Operations)` (x)`  
 - [`Cosine`](./Unary-Operations)` (x)`  
 - [`Clip`](./Unary-Operations)` (x, minValue, maxValue)`  
 - [`Exp`](./Unary-Operations)` (x)`  
 - [`Floor`](./Unary-Operations)` (x)`  
 - [`Log`](./Unary-Operations)` (x)`  
 - [`Negate`](./Unary-Operations)` (x)`  
   [`-x`](./Unary-Operations)  
 - [`BS.Boolean.Not`](./Unary-Operations)` (b)`  
   [`!x`](./Unary-Operations)  
 - [`Reciprocal`](./Unary-Operations)` (x)`  
 - [`Round`](./Unary-Operations)` (x)`  
 - [`Sin`](./Unary-Operations)` (x)`  
 - [`Sqrt`](./Unary-Operations)` (x)`  

#### Elementwise operations, binary
 - [`ElementTimes`](./Binary-Operations)` (x, y)`  
   [`x .* y`](./Binary-Operations)  
 - [`Minus`](./Binary-Operations)` (x, y)`  
   [`x - y`](./Binary-Operations)  
 - [`Plus`](./Binary-Operations)` (x, y)`  
   [`x + y`](./Binary-Operations)`  
 - [`LogPlus`](./Binary-Operations)` (x, y)`  
 - [`Less`](./Binary-Operations)` (x, y)`  
 - [`Equal`](./Binary-Operations)` (x, y)`  
 - [`Greater`](./Binary-Operations)` (x, y)`  
 - [`GreaterEqual`](./Binary-Operations)` (x, y)`  
 - [`NotEqual`](./Binary-Operations)` (x, y)`  
 - [`LessEqual`](./Binary-Operations)` (x, y)`  
 - [`BS.Boolean.And`](./Binary-Operations)` (a, b)`  
   [`BS.Boolean.Or`](./Binary-Operations)` (a, b)`  
   [`BS.Boolean.Xor`](./Binary-Operations)` (a, b)`  
...
#### Matrix product and convolution operations
 - [`Times`](./Times-and-TransposeTimes)`(A, B, outputRank=1)`  
   [`A * B`](./Times-and-TransposeTimes)  
 - [`TransposeTimes`](./Times-and-TransposeTimes)`(A, B, outputRank=1)`  
 - [`Convolution`](./Convolution)`(weights, x, kernelShape, mapDims=(0), stride=(1), sharing=(true), autoPadding=(true), lowerPadding=(0), upperPadding=(0), imageLayout='CHW', maxTempMemSizeInSamples=0)`  
 - [`Pooling`](./Pooling)`(x, poolKind/*'max'|'average'*/, kernelShape, stride=(1), autoPadding=(true), lowerPadding=(0), upperPadding=(0), imageLayout='CHW')`  

#### Learnable parameters and constants
 - [`ParameterTensor`](./Parameters-And-Constants#parametertensor)` {shape, learningRateMultiplier=1.0, init='uniform'/*|gaussian*/, initValueScale=1.0, initValue=0.0, randomSeed=-1, initFromFilePath=''}`  
 - [`Constant`](./Parameters-And-Constants#constant)` {scalarValue, rows = 1, cols = 1}`  
...
#### Loss functions and metrics
 - [`CrossEntropyWithSoftmax`](./Loss-Functions-and-Metrics#crossentropy-crossentropywithsoftmax)` (targetDistribution, nonNormalizedLogClassPosteriors)`  
   [`CrossEntropy`](./Loss-Functions-and-Metrics#crossentropy-crossentropywithsoftmax)` (targetDistribution, classPosteriors)`  
 - [`Logistic`](./Loss-Functions-and-Metrics#logistic-weightedlogistic)` (label, probability)`  
   [`WeightedLogistic`] (./Loss-Functions-and-Metrics#logistic-weightedlogistic)`(label, probability, instanceWeight)`  
 - [`ClassificationError`](./Loss-Functions-and-Metrics#classificationerror)` (labels, nonNormalizedLogClassPosteriors)`  
 - `MatrixL1Reg(matrix)`  
 - `MatrixL2Reg(matrix)`  
 - `SquareError (x, y)`  

#### Reductions
 - [`ReduceSum`](./Reduction-Operations)` (z, axis=None)`  
   [`ReduceLogSum`](./Reduction-Operations)` (z, axis=None)`  
   [`ReduceMean`](./Reduction-Operations)` (z, axis=None)`  
   [`ReduceMin`](./Reduction-Operations)` (z, axis=None)`  
   [`ReduceMax`](./Reduction-Operations)` (z, axis=None)`  
 - `CosDistance (x, y)`  
 - `SumElements (z)`  

#### Training operations
 - [`BatchNormalization`](./BatchNormalization)` (input, scale, bias, runMean, runInvStdDev, spatial, normalizationTimeConstant = 0, blendTimeConstant = 0, epsilon = 0.00001, useCntkEngine = true, imageLayout='CHW')`  
 - [`Dropout`](./Dropout) `(x)`  
...
#### Reshaping operations
 ...
 - [`BS.Sequences.Gather (where, x)`](Gather-and-Scatter#gather-scatter)  
   [`BS.Sequences.Scatter (where, y)`](Gather-and-Scatter#gather-scatter)  
   [`BS.Sequences.IsFirst (x)`](Gather-and-Scatter#isfirst-islast)  
   [`BS.Sequences.IsLast (x)`](Gather-and-Scatter#isfirst-islast)  
...
#### Model editing
 ...
 - [`BS.Network.CloneFunction (inputNodes, outputNodes, parameters="learnable" /*|"constant"|"shared"*/)`](./CloneFunction)
...
#### Deprecated
 - [`ErrorPrediction`](./Loss-Functions-and-Metrics#classificationerror)` (labels, nonNormalizedLogClassPosteriors)`  
